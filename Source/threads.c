...
osMessageQueueId_t ADC_messageQ;

void Create_OS_Objects(void) {
	LCD_mutex = osMutexNew(&LCD_mutex_attr);
	
	// Creates a new message queue
	// TODO Need to initialize the message queue
	ADC_messageQ = osMessageQueueNew(8,sizeof(adc_message_t),NULL);

	// Creates the different threads that will run 
