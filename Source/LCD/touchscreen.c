...
extern osThreadId_t t_Read_TS, t_Read_Accelerometer, t_Sound_Manager, t_US, t_Refill_Sound_Buffer, t_BUS;
extern osMessageQueueId_t ADC_messageQ;
...
uint32_t LCD_TS_Read(PT_T * position) {
	...
	if (b>0) {
		...
		// enqueue these two conversion requests
		adc_message_t adc_msg_1;
		//adc_msg = { .ChannelNum = LCD_TS_YU_CHANNEL, .ResultPtr = (uint16_t *) &yu, .TID = t_Read_TS, .FlagNum = 0x1 };
		adc_msg_1.ChannelNum = LCD_TS_YU_CHANNEL;
		adc_msg_1.ResultPtr = &yu;
		adc_msg_1.TID = t_Read_TS;
		adc_msg_1.FlagNum = 0x01;
		
		DEBUG_START(DBG_3);
		osMessageQueuePut(ADC_messageQ, &adc_msg_1, NULL, osWaitForever);
		osThreadFlagsWait(adc_msg_1.FlagNum, NULL, NULL); // Last param is a timeout
		DEBUG_TOGGLE(DBG_3);
		/* To be deleted
		// Read inputs
		...
		*/
		...
		xl = ADC0->R[0];
		*/
		adc_message_t adc_msg_2;
		adc_msg_2.ChannelNum = LCD_TS_XL_CHANNEL;
		adc_msg_2.ResultPtr = &xl;
		adc_msg_2.TID = t_Read_TS;
		adc_msg_2.FlagNum = 0x10;
		
		DEBUG_START(DBG_3);
		osMessageQueuePut(ADC_messageQ, &adc_msg_2, NULL, osWaitForever);
		osThreadFlagsWait(adc_msg_2.FlagNum, NULL, NULL);
		y = xl;
		DEBUG_STOP(DBG_3);
