...
// variables for the message queue
volatile adc_message_t g_msg;
...
void ADC0_IRQHandler() {
	int msg_count;
	static char priority_type = TYPE_PRIORITY;
	...
	switch(priority_type)
	{
		case TYPE_PRIORITY:
			...
			break;
		case TYPE_QUEUED_REQ:
			//DEBUG_START(DBG_1);
			// get data from adc
			*g_msg.ResultPtr = ADC0->R[0];
		  // set flag
			osThreadFlagsSet(g_msg.TID, g_msg.FlagNum);
			//DEBUG_STOP(DBG_1);
			break;
	}
	// TODO finish the rest of the structure
	
	msg_count = osMessageQueueGetCount(ADC_messageQ);
	if(msg_count > 0)
	{
		priority_type = TYPE_QUEUED_REQ;
		osMessageQueueGet(ADC_messageQ, &g_msg, NULL, NULL);
		// function for software tiggering
		Config_SW_Trig(g_msg.ChannelNum);
	}
	else
	{
		priority_type = TYPE_PRIORITY;
		Config_HW_Trig();
	}
}
...
void Config_SW_Trig(uint8_t channel_num)
{
	ADC0->SC2 &= ~ADC_SC2_ADTRG(1);
	// config channel
	ADC0->SC1[0] &= ~ADC_SC1_ADCH_MASK;
	ADC0->SC1[0] |= ADC_SC1_ADCH(channel_num);
}

void Config_HW_Trig(void)
{
